...
jQuery(function ($) {
    rm_single_product = {
        ...
        dateFormatInitial: function () {
            ...
            let allFormatsDate = {
                'MM/DD/YYYY': 'MM/DD/YYYY', // Works
                'DD/MM/YYYY': 'DD/MM/YYYY', // Not
                'DD MMM YYYY': 'DD MMM YYYY', // Not
                'YYYY-MM-DD': 'YYYY-MM-DD', // Not
                'MM-DD-YYYY': 'MM-DD-YYYY' // Not in admin
            };
        ...
        load_default_values() {
            ...
            this.rent_start = $('#exact-rent-start').val() || moment().format(this.dateFormatInitial());
        ...
        load_configuration() {
            ...
            $(document).ready(function(){
                ...
                $('.rentmy-custom-fields label.radio-container').first().trigger('click');
            });
        },
...
*/
            ...
            var dateFormat = rm_single_product.dateFormatInitial();
            let data = {
                ...
                data: {
                    ...
                    rent_start: moment(this.rent_start).format('YYYY-MM-DD HH:mm'),
                    rent_end: moment(this.rent_end).format('YYYY-MM-DD HH:mm'),
                    rental_type: this.rental_type
                }
            };

            if( data.data.rent_end == "Invalid date" ) {
                data.data.rent_end = '';
            }
            ...
            if (!custom_fields.length){
                ...
                custom_fields = $("select[name='customFields[]'], input[name='customFields[]']:checked")
                //custom_fields = $("input[name='customFields[]']:checked")
                    .map(function(){
                        if ($(this).val() != ''){
            ...
            if(!rm_single_product.loop) {
                rm_single_product.loop = 0;
            }
            rm_single_product.loop++;
            if( rm_single_product.loop > 20 ) {
                return false;
            }


                if (response.status == 'OK' && response.result.data && response.result.available ) {
                    ...
                    if (response.result.available > 0) {
                        ...
                        if( jQuery('.rm-date-start-box').css('display') == 'none'
                            && jQuery('#rentmy-rent-item').prop('disabled') == true
                            && rm_single_product.config.show_earliest_start_date == 0
...
/*
                    if (($('#rm-date').length > 0) && (typeof response.result.start_date != 'undefined') && (typeof response.result.end_date != 'undefined')) {
                        if ($('#rm-date').attr('disabled')) {
                            ...
                            let startDate = moment(response.result.start_date, 'YYYY-MM-DD HH:mm').format(dateFormat);
                            ...
                            let endDate = moment(response.result.end_date, 'YYYY-MM-DD HH:mm').format(dateFormat);

                            if( endDate != 'Invalid date' ) {
                                jQuery('.daterangeEnd').val( endDate ).data('daterangepicker').setStartDate( endDate );
...
*/
                ...
                if (response && response.result && (response.result.error || response.result.message) ) {
                    if( response.result.message == "Rental time(s) were adjusted for store open hours" ) {

                        $('.show_earliest_start_date_box .rent_input').eq(0).hide();

                        let dateFormat = rm_single_product.dateFormatInitial();
                        let startDate = moment(rm_single_product.rent_start, 'YYYY-MM-DD HH:mm').add(1, 'd').format(dateFormat);
                        ...
                        let endDate = moment(rm_single_product.rent_end, 'YYYY-MM-DD HH:mm').add(1, 'd').format(dateFormat);
        ...
        get_dates_from_duration: function () {
            let singleDateRange = $('.single-date-range');
            //let durationTime = $('.exact-date-wrapper #duration');
            let durationTime = $('.duration_box .duration:checked');
            let exactTime = $('.exact-date-wrapper #exact_time');
            let addToCartBtn = $('#rentmy-rent-item');
            let dateFormat = 'YYYY-MM-DD';
            let symbolPrice = rm_single_product.config.currency_format.symbol;
            ...
            let data = {
                ...
                data: {
                    ...
                    type: $.trim( durationTime.attr('data-type') )
                }
            };
...
//data.data.type = "day";
            ...
            jQuery.post(rentmy_ajax_object.ajaxurl, data, function (response) {
                ...
                rm_single_product.get_price_value(true, data.data.custom_fields);
        ...
        init_daterange_picker: function (configurations) {
            ...
            if (dateConfigureObj.singleDatePicker) {
                formated_date = this.convertDateFormat(startDate, this.dateFormatInitial());
            ...
            if (type == 'buy') {
                ...
                if( custom_fields ) {
                    custom_fields.forEach( function (field){
                        rm_single_product.price = parseFloat(rm_single_product.price) + parseFloat(field['price_amount']);
        ...
        bind_select_options_modern: function (el_id, json) {
            if (json && json.length > 0) {
                $VariantSetEl = jQuery('.variantSetBox' + el_id);
                $VariantSetEl.find('.radio-container').remove();
                for (var i = 0; i < json.length; i++) {
                    var active = (i == 0) ? 'active' : '';
                    var checked = (i == 0) ? 'checked="checked"' : '';
                    var optionHtml = jQuery('<label class="radio-container ' + active + '" for="product_field_' + i + '"><input type="radio" name="variantSets[' + json[i]['variant_set_id'] + ']" value="' + json[i]['id'] + '" id="variantSet_' + json[i]['id'] + '" ' + checked + '>' + json[i]['name'] + '<span class="checkmark"></span></label>');
                    //var optionHtml = jQuery('<label class="radio-container" for="product_field_' + i + '"><input type="radio" name="variantSets[' + json[i]['variant_set_id'] + ']" value="' + json[i]['id'] + '" id="variantSet_' + json[i]['id'] + '">' + json[i]['name'] + '<span class="checkmark"></span></label>');
                    $VariantSetEl.append(optionHtml);
                }

                $VariantSetEl.find('.radio-container').first().trigger('click');
       ...
       let link = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mPkrQcAAJ8AjsW513IAAAAASUVORK5CYII=';
        ...
        bind_product_form_options: function (json) {
            ...
            this.show_priceoptions(prices, json.available);
        ...
        show_price: function (prices) { // when variants changed show main price
            if (prices.rent.type) { // rent price radio
                if (prices.rent.price.length > 0) {
                    ...
                    $('.price .rent').html("<h6><span class='pre'>$</span><span class='amount'> " + prices.rent.price[0].price + "</span></h6>");
                    if (this.rental_type == 'rent'){
                        rm_single_product.checked_rent_buy_radio('rent');
                    }
                }
            }
            if (prices.buy.type && prices.buy.price) { // buy price radio
                this.price = this.initial_buy_price = prices.buy.price;
                $('.price .buy').html("<h6><span class='pre'>$</span><span class='amount'> " + prices.buy.price + "</span></h6>");
                if (this.rental_type == 'buy'){
        ...
        show_price_types: function (prices) {
            ...
            if (prices.buy.type) {
                ...
                if (!prices.rent.type){
                    ...
                    $('.rental-type .switch').show();
                }
            }

        },
        show_priceoptions: function (prices, available) {
            if (prices.rent.type && !this.recurring) {
                ...
                if( !cart_added_recurring.duration_type ) is_rental_type_match = true;
                ...
                $.each(prices.rent.price, function (index, rents) {
                    ...
                    duration_label = rents.duration?(config_labels?.product_details?.lbl_billing_at_a_rate_of?config_labels?.product_details?.lbl_billing_at_a_rate_of:' ') + ' ' + rm_single_product.priceFormat(rents.price) + ' ' + label_for_per + ' '+ rents.label:'';
                    duration_label = rents.duration + " " + rents.label  + "<br />$" + rm_single_product.priceFormat(rents.price);
                    ...
                    if(index == 0) priceOptions += '<label>Rental date range</label><br />';
                    if (cart_without_recurring) {



                        priceOptions += `<label class="radio-container ${active}">
                                                ...
                                                <input type="radio" name="rental-price" class="${index==0?'first-element-selection':''}"
                                                       ...
                                                       value="${rents.id}" ${index==0?'checked':''} onclick="addActiveToParent(this)" >
                        ...
                        priceOptions += `
                                ...
                                <label class="radio-container" style="display: none">
                                                ...
                                                <input type="radio" name="rental-price" class="${index==0?'first-element-selection':''}"
                                                       ...
                                                       data-start_date="${rents.rent_start}"
                                                       data-end_date="${rents.rent_end}"
                                                       value="${rents.id}" ${cart_added_recurring.duration_type == rents.duration_type?'checked':''} onclick="addActiveToParent(this)>
                ...
                if ( available > 0 && is_rental_type_match){
                if( jQuery('.rm-date-start-box').css('display') == 'none'
                    && jQuery('#rentmy-rent-item').prop('disabled') == true
                    && rm_single_product.config.show_earliest_start_date == 0
                    && rm_single_product.config.show_start_date == true
                    ...
                    $('.availability .availability-count').html(available);
                }else{
        ...
        update_quantity(type, counter) {
            ...
            if (type == 'increase') {
                this.quantity = parseInt(this.quantity) + parseInt(counter);
                $('#rm_quantity').val(this.quantity);
            ...
            this.set_price();
            if (jQuery('.availability-count').html() > 0) {
                var availability_count = jQuery('.availability-count').html();
            }else {
                var availability_count = rm_single_product.available; //jQuery('.availability-count').html();
            }

            if( parseInt(availability_count) >= parseInt(this.quantity) ) {

                if( jQuery('#rentmy-rent-item').prop('disabled') == true
                    && rm_single_product.config.show_earliest_start_date == 0
                    && rm_single_product.config.show_start_date == true
                    && rm_single_product.rental_type == 'rent' ) {
                    // Do not enable button
        ...
        add_to_cart: function (event='') {
                ...
                if (!data.data.rent_start || !data.data.rent_end) {
                    if ($('#rm-date').length > 0) {
                        ...
                        if ($('#rm-date').attr('disabled')) {
                            ...
                            var rmStartDate = jQuery('#rm-date').data('daterangepicker').startDate;
                            var dateFormat = rm_single_product.dateFormatInitial()
                            if (rmStartDate && rmStartDate._i) {
                                data.data.rent_start = moment(rmStartDate._i, dateFormat).format('YYYY-MM-DD HH:mm');
                            } else {
                                data.data.rent_start = rmStartDate.format('YYYY-MM-DD HH:mm');
                            }

                            var rmEndDate = jQuery('#rm-date').data('daterangepicker').endDate;
                            if (rmEndDate && rmEndDate._i) {
                                data.data.rent_end = moment(rmEndDate._i, dateFormat).format('YYYY-MM-DD HH:mm')
                            } else {
                                data.data.rent_end = rmEndDate.format('YYYY-MM-DD HH:mm');
                            }

                            var durationChecked = jQuery(".duration:checked").val();
                            var durationType = jQuery(".duration:checked").data('type');

                            if (durationChecked !== undefined) {
                                data.data.rent_start = moment(jQuery('#rm-date-start').val(), dateFormat).format('YYYY-MM-DD') + " " + jQuery(".timeButton:checked").val();
                                data.data.rent_start = moment(data.data.rent_start, 'YYYY-MM-DD hh:mm A').format('YYYY-MM-DD HH:mm');
                                data.data.rent_end = moment(data.data.rent_start, 'YYYY-MM-DD hh:mm A').add(parseInt(durationChecked), durationType).format('YYYY-MM-DD HH:mm');
                            }
            ...
            if( data.data.rent_end == 'Invalid date' ) {
                var dateFormat = rm_single_product.dateFormatInitial()
                data.data.rent_end = moment( jQuery('#rm-date-end').val(), dateFormat ).format('YYYY-MM-DD HH:mm');
            }

            if( data.data.rent_start == 'Invalid date' ) {
                var dateFormat = rm_single_product.dateFormatInitial()
                data.data.rent_start = moment( jQuery('#rm-date-start').val(), dateFormat ).format('YYYY-MM-DD HH:mm');
            }
        ...
        get_dates_price_duration: function (price_id) {
            ...
            if( rm_single_product.rent_start == "Invalid date" ) {
                rm_single_product.rent_start = moment(jQuery('#rm-date-start').data('daterangepicker').startDate, 'MM-DD-YYYY hh:mm A').format('YYYY-MM-DD HH:mm');
            }
            ...
            jQuery.post(rentmy_ajax_object.ajaxurl, data, function (response) {
                ...
                try {
                    ...
                    $('.daterangeStart').data('daterangepicker').setStartDate(moment(response.start_date).format(dateFormat));
                    $('.daterangeEnd').data('daterangepicker').setStartDate(moment(response.end_date).format(dateFormat));
    ...
    $('body')
        ...
        .on('click', ".variantSets .radio-container", function (e) {
            var box_el = jQuery(this).parent('.variantSets');
            index = box_el.attr('data-index');
            total = box_el.attr('data-total');
            var input_el = box_el.find('input:checked');

            jQuery('#togBtn').prop('checked', true);

            if (index != total) {
                current_set_id = box_el.attr('data-id');
                next_set_id = box_el.attr('data-next-id');
                var data = {
                    action: 'rentmy_options',
                    action_type: 'get_variant_chain',
                    data: {
                        product_id: $('#rm_pd_product_id').val(),
                        variant_id: input_el.val(),
                    }
                };
                jQuery.post(rentmy_ajax_object.ajaxurl, data, function (response) {
                    rm_single_product.bind_select_options_modern(next_set_id, response);
                });
            } else { // last item of the chain
                prev_set_id = box_el.attr('data-prev-id');

                var chain_id = [];
                jQuery('.variantSets').find('input:checked').each(function() {
                    this_val = jQuery(this).val();
                    if( this_val != input_el.val() ) {
                        chain_id.push(this_val);
                    }
                });
                var chain_id = chain_id.join('-');
                var data = {
                    action: 'rentmy_options',
                    action_type: 'get_last_variant',
                    data: {
        ...
        .on('click', ".rentmy-custom-fields label.radio-container", function () {
            let custom_fields = [];
            $(".rentmy-custom-fields input[name='customFields[]']:checked").each(function() {
                if ($(this).val() != '') {
                    let data = $(this).val();
                    data = data.split("%rentmy%").join(" ");
                    custom_fields.push(JSON.parse(data));
        ...
        .on('keyup', "#rm_quantity", function (e) {
            rm_single_product.update_quantity( 'input', parseInt(e.target.value) )
        })
