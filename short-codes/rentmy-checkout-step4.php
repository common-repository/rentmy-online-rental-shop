...
function rentmy_checkout_complete_template()
{
    if (isset($_GET['order_id']) && !empty($_GET['order_id'])){
        $_SESSION['order_uid'] = $_GET['order_id'];
    }
...
<div class="rentmy-plugin-manincontent">
    <div class="checkout-complete">
        ...
        */?>
        ...
        <span
            class="rentmy-success-msg"><?php echo parsingThankYouVariable(($order_data['type']===1) ? $payment_label['lbl_thanks_confirm_order'] : $payment_label['lbl_thanks_confirm_quote'], $order_data);?></span>
...
add_shortcode('rentmy-order-complete', 'rentmy_checkout_complete_template');

function parsingThankYouVariable($str, $order_data){
    $first_name = $order_data['first_name'];
    $last_name = $order_data['last_name'];
    if (strpos($str, '%first_name%') > 0){
        $str = str_replace('%first_name%', $first_name, $str);
    }
    if (strpos($str, '%last_name%') > 0){
        $str = str_replace('%last_name%', $last_name, $str);
    }
    if (strpos($str, '%full_name%') > 0){
        $fullName = "";
        if (!empty($first_name))
            $fullName.=$first_name;
